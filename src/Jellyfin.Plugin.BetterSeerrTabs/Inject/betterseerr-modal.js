...
(function () {
    ...
    function fetchTmdbLogoPath(mediaId, mediaType, apiKey) {
        ...
        return fetchTmdbJson(filteredUrl, apiKey)
            ...
            .catch(function (err) {
                console.warn('BetterSeerrTabs: TMDB logo fetch failed', err);
                return null;
            });
    }
    ...
    function fetchTmdbDetailsFromBrowser(mediaId, mediaType, apiKey) {
        ...
        return Promise.all([
            ...
            console.warn('BetterSeerrTabs: TMDB details fetch failed', err);
            return null;
        });
    }
    ...
    function submitRequest(mediaId, mediaType, option, onSuccess) {
        ...
        return ApiClient.ajax({
            ...
            if (response && response.errors && response.errors.length > 0) {
                ...
                console.error('BetterSeerr request:', response);
                return;
            }
            ...
            console.error('BetterSeerr request:', err);
    ...
    function openQualityModal(mediaId, mediaType, title, onSuccess, is4k, selectedSeasons) {
        ...
        ApiClient.ajax({
            url: ApiClient.getUrl('BetterSeerrTabs/request-options/' + mediaType),
            type: 'GET',
            dataType: 'json'
            ...
            console.error('BetterSeerr profiles:', err);
    ...
    function openDetailsModal(mediaId, mediaType) {
        ...
        loadModalDetails(mediaId, mediaType).then(function (data) {
            ...
            console.error('BetterSeerr modal:', err);
            closeDetailsModal();
            Dashboard.alert('Failed to load details');
        });
    }

    window.betterSeerrModal = {
