...
(function () {
    ...
    function getFetchSize() {
        return Number(getRequestsAdvanced().fetchSize) || DEFAULT_FETCH_SIZE;
    }
    ...
    function fetchAllRequests() {
        ...
        function loadNext() {
            return fetchRequestPage(skip).then(function (data) {
                ...
                const loadedPages = Math.ceil(all.length / fetchSize);
    ...
    function bindContainerEvents(container) {
        ...
        container.addEventListener('click', function (event) {
            ...
            const totalPages = Math.max(1, Math.ceil(getFilteredRequests().length / getPageSize()));
